...
void SampleChopperAudioProcessorEditor::timerCallback()
{
    ...
    //setting playhead positions in waveformDisplay
    for(int i = 0; i < bankList.size(); i++)
    {
        waveformDisplay.setPlayheadPos(bankList[i]->getPositionRelative(), i + 1);
    }
    for(int i = 0; i < bankList.size(); i++)
    {
        waveformDisplay.setPlayheadPos(bankList[i]->getPositionRelative(), i + 1);
...
void SampleChopperAudioProcessorEditor::mouseDrag(const juce::MouseEvent& event)
{
    if(&waveformDisplay == event.eventComponent)
    {
        ...
        //updating parameters when waveform display is the component
        //will have to have a button to manually control ADSR or sample selection
        if(event.getMouseDownY() < waveformDisplay.controllerStartY)
        {
            ...
            double normalisedStart = waveformDisplay.waveformStart / fileLength; //proportion of the waveform
            double normalisedEnd = waveformDisplay.waveformEnd / fileLength;
