...
void WaveformDisplay::drawPlayheads(std::vector<Interval<float>> loopRegions, std::vector<float> bankPositions, juce::Graphics& g)
{
    ...
    for(int i = 0; i < bankPositions.size(); i++)
    {
        ...
        //DBG("Bank " << char(i + 1) << "current position =  " << bankPositions[i]);
        g.setColour(myColours[i]);
